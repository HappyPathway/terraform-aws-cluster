      ...
      dynamic "launch_template" {
        ...
        content {
          launch_template_specification {
            launch_template_id = var.launch_template.create && local.launch_template != null ? local.launch_template.id : var.auto_scaling.mixed_instances_policy.launch_template.launch_template_specification.launch_template_id
            version            = var.launch_template.create && local.launch_template != null ? local.launch_template.version : var.auto_scaling.mixed_instances_policy.launch_template.launch_template_specification.version
          }
        }
      }
    }
...
locals {
  autoscaling_groups = {
    ...
    resource    = one(aws_autoscaling_group.asg)
  }
  autoscaling_group = var.auto_scaling.create ? lookup(local.autoscaling_groups, "resource") : lookup(local.autoscaling_groups, "data_source")
}

    ...
    dynamic "launch_template" {
      ...
      content {
        launch_template_specification {
          launch_template_id = var.auto_scaling.mixed_instances_policy.launch_template.launch_template_specification.launch_template_id
          version            = var.auto_scaling.mixed_instances_policy.launch_template.launch_template_specification.version
        }
      }
    }
  }
...
resource "aws_autoscaling_group" "asg_launch_configuration" {
  count = var.auto_scaling.create && var.auto_scaling.configuration_type == "launch_configuration" ? 1 : 0
  name  = var.project_name

  launch_configuration = local.launch_configuration.name
...
locals {
  autoscaling_groups = {
    ...
    launch_configuration   = one(aws_autoscaling_group.asg_launch_configuration)
  }
